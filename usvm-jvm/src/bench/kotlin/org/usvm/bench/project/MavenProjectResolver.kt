...
class MavenProjectResolver(
    ...
    private fun registerModuleFromDirectory(directory: Path, isTestModule: Boolean) {
        if (!directory.isDirectory()) {
            return
        }
        val classesSnapshotDir = resolverDir.resolve("classes_${resolvedModules.size}")
        directory.copyToRecursively(classesSnapshotDir, followLinks = false, overwrite = false)
        resolvedModules += ProjectResolver.ProjectModuleClasses(isTestModule, directory, listOf(classesSnapshotDir))
    }

    @OptIn(ExperimentalPathApi::class)
    private fun buildProject(): Boolean {
        val args = listOf(MAVEN_EXECUTABLE_NAME) + listOf("clean", "test") + mavenCommandFlags

        javaToolchain = ProjectResolver.tryJavaToolchains { ProjectResolver.runCommand(projectSourceRoot, args, it) } ?: return false

        projectSourceRoot.visitFileTree {
            onPreVisitDirectory { directory, _ ->
                if (isMavenProjectRoot(directory)) {
                    val targetDirectory = directory.resolve("target")
                    registerModuleFromDirectory(targetDirectory.resolve("classes"), isTestModule = false)
                    registerModuleFromDirectory(targetDirectory.resolve("test-classes"), isTestModule = true)
                }
    ...
    private fun resolveDependencies(): Boolean {
        ...
        val args = listOf(MAVEN_EXECUTABLE_NAME) + mavenCommandFlags + listOf(
            DEPGRAPH_PLUGIN_ID,
            //"-DexcludeOptionalDependencies=false",
            "-DclasspathScope=test",
