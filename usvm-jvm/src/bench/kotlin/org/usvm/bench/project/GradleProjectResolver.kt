...
class GradleProjectResolver(
    ...
    private fun registerModule(moduleRoot: Path, snapshotLibs: (Path) -> List<Path>) {
        ...
        snapshotLibs(snapshotDir).map { lib ->
            val isTest = lib.endsWith("test")
            val classesSnapshotDir = resolverDir.resolve("classes_${resolvedModules.size}")
            lib.copyToRecursively(classesSnapshotDir, followLinks = false, overwrite = false)
            resolvedModules += ProjectResolver.ProjectModuleClasses(
                isTestModule = isTest,
                moduleRoot,
                listOf(classesSnapshotDir)
            )
        }
    }
    ...
    private fun buildProject(): Boolean {
        ...
        val buildTarget = "testClasses" // TODO: will it build ALL main classes?
