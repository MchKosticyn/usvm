...
import org.usvm.machine.JcCallGraphStatistics
import org.usvm.machine.JcComponents
import org.usvm.machine.JcContext
import org.usvm.machine.JcDebugProfileObserver
...
data class JcBenchMachineResult(
    val states: List<JcState>,
    val coverage: Float,
...
class JcBenchMachine(
    cp: JcClasspath,
    private val options: UMachineOptions,
    private val onCriticalFail: (JcState, Throwable) -> Unit,
    ...
    fun analyze(methods: List<JcMethod>, targets: List<JcTarget> = emptyList()): JcBenchMachineResult {
        ...
        val methodsToTrackCoverage: Set<JcMethod> =
            when (options.coverageZone) {
                ...
                else -> throw IllegalStateException("Coverage zone ${options.coverageZone} not supported in JcPlainMachine")
            }
        ...
        val criticalFailObserver = object : UMachineObserver<JcState> {
            override fun onCriticalFail(state: JcState, exception: Throwable) {
                this@JcBenchMachine.onCriticalFail(state, exception)
            }
        }
        ...
        observers.add(criticalFailObserver)
        ...
        while (!pathSelector.isEmpty()) {
            pathSelector.remove(pathSelector.peek())
            statesCount++
        }

        return JcBenchMachineResult(
            statesCollector.collectedStates,
            coverageStatistics.getTotalCoverage(),
            timeStatistics.runningTime.inWholeMilliseconds,
            stepsStatistics.totalSteps.toInt(),
            statesCount
        )
    }

    fun analyze(method: JcMethod, targets: List<JcTarget> = emptyList()): JcBenchMachineResult =
