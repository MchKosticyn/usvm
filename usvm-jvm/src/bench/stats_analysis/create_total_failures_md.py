...
markdown_path = "total_failures.md"

# Prepare data for Markdown
exception_data = (
    df_filtered.groupby(["exception", "project"])  # Group by exception and project
    .agg(
        methods=("method", lambda x: list(set(x)))  # Deduplicate methods
    )
    .reset_index()
)
# Calculate total method count for sorting exceptions
exception_totals = (
    exception_data.groupby("exception")
...
# Path for the total failures Markdown file
markdown_path = "total_failures_trimmed_3.md"


# Function to trim exception text
def trim_exception_text(exception, max_length=300):
    if len(exception) > max_length:
        return exception[:max_length]
    return exception


# Start writing Markdown content
with open(markdown_path, "w") as md_file:
    md_file.write("# Total Failures\n\n")
    ...
    for _, row in exception_totals.iterrows():
        ...
        trimmed_exception = trim_exception_text(exception)

        md_file.write(f"## Exception:\n")
        md_file.write(f"```\n{trimmed_exception}\n```\n")
        #md_file.write(f"- **Total Methods**: {total_methods}\n\n")

        exception_group = exception_data[exception_data["exception"] == exception]
