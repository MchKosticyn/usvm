...
import org.usvm.machine.interpreter.transformers.springjpa.generatedStaticSerializer
...
abstract class Join(
    ...
    private fun resolveComplexTarget(ctx: MethodCtx, name: String, root: JcLocalVar): JcLocalVar {
        ...
        val mapperLambda = ctx.genCtx.generateLambda(ctx.cp, "fltMap#${ctx.getLambdaName()}", mapper)
    ...
    class FlatFeature(val info: CommonInfo, val name: String, val join: Join) : JcBodyFillerFeature() {
        ...
        override fun JcSingleInstructionTransformer.BlockGenerationContext.generateBody(method: JcMethod) {
            ...
            val rootSer = (obj.type as JcClassType).declaredMethods.single { it.method.generatedStaticSerializer }
                .let { ctx.genCtx.generateLambda(ctx.cp, "rootSerializer", it.method) }
            val targetSer = field.typeName.genericTypes[0].let { ctx.cp.findType(it) as JcClassType }
                ...
                .let { ctx.genCtx.generateLambda(ctx.cp, "targetSerializer", it.method) }
